"""Event publisher — Sentinel's only outbound channel.

Sentinel publishes standardized events to Hermes' ``POST /event`` endpoint.
It never contacts Discord, Telegram or any other notification provider
directly. When Hermes is disabled or unreachable, events are dropped after a
few retries and logged — observation must never block on notification.
"""
